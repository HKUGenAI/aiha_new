# https://github.com/opendatalab/MinerU/blob/master/magic_pdf/layout/layout_sort.py
"""对pdf上的box进行layout识别，并对内部组成的box进行排序."""

from bbox_sort import (CONTENT_IDX, CONTENT_TYPE_IDX,
                                        X0_EXT_IDX, X0_IDX, X1_EXT_IDX, X1_IDX,
                                        Y0_EXT_IDX, Y0_IDX, Y1_EXT_IDX, Y1_IDX,
                                        paper_bbox_sort)
from layout_detect_utils import (
...
from utils.boxbase import get_bbox_in_boundary
        ...
        if layout_type == LAYOUT_UNPROC:  # 说明是非独占单行的，这些需要垂直切分
            ...
            if len(v_split_layouts) == 1:
                if len(v_split_layouts[0]['sub_layout']) == 0:
                    ...
                    # print(f"WARNING: pageno={page_num}, 无法分割的layout: ", v_split_layouts)
            """
            组合起来最终的layout
            """
            sorted_layouts[i] = {
...
def sort_with_layout(bboxes: list, page_width, page_height) -> Tuple[list, list]:
    """输入是一个bbox的list.
